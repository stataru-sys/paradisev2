...
## Профили грузятся из DatingProfiles (50 анкет + фото).
...
@export var photo_path: NodePath
...
@onready var _photo: TextureRect = get_node(photo_path) as TextureRect
...
func _ready() -> void:
	...
	_profiles = preload("res://scripts/run1/DatingPool.gd").get_all().duplicate(true)
	_profiles.shuffle()
	_show_current()
...
func _show_current() -> void:
	...
	if int(_run.energy) <= 0:
		_like_button.disabled = true
		_dislike_button.disabled = true
	var p: Dictionary = _profiles[_index]
	_name_label.text = String(p["name"])
	_bio_label.text = String(p["bio"])
	var photo_res: String = String(p.get("photo", ""))
	if photo_res != "":
		var tex: Texture2D = load(photo_res) as Texture2D
		_photo.texture = tex
		_photo.visible = true
	else:
		_photo.visible = false
