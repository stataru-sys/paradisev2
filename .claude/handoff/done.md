...
**Вердикт:** Слайс принят. Код чистый, следует паттернам WorkProgram/WorkHub/MailSortGame. Рефактор `_start_work_game(scene, energy)` + обобщённый `_lock_card(card, btn)` — ровно как просил `next.md`, дублирование убрано. Все 13 пунктов acceptance проходят. Контент 12 багов — попадание в сатиру (особенно «Деньги уходят в минус / Добавить clamp в Economy.add», ловушки «Сказать что это фича», «Назвать вечным двигателем»). Out-of-scope соблюдён: новых сигналов нет, `comment_override` — параметр, а не сигнал.

**Ревью вскрыло 2 проблемы — НЕ регрессии Slice J** (coder реализовал `next.md` дословно), но требуют отдельного фикс-слайса перед Slice K:

1. **🔴 Деньги мини-игр косметические.** `BugfixGame` и `MailSortGame` не вызывают `Economy.add()` — `_earned` идёт только в WorkResult и `work_day_finished.emit()`, а `Run1.gd`/`Run2.gd._on_work_day_finished` = `pass`. Игрок видит «+20$», но `Economy.money` не растёт. Заработок зачисляет только `WorkProgram` (сортировка) напрямую. Баг с Slice I, пропущен в ревью Slice I. Фикс — централизованный обработчик `_on_work_day_finished(earned)` в Run-контроллерах; **осторожно:** `WorkProgram` уже зачисляет live, иначе сортировка задвоится. Нужно решение: либо все 3 мини-игры зачисляют сами, либо все через центр.
2. **🟡 Инверсия баланса.** Ошибка стоит −1$, но плодит баг ценой +5$ → нетто +4$ за ошибку. Итог = 15 + 4×ошибок (бонус +5$ только при 0 ошибок): 0 ошибок → 20$, 2 ошибки → 23$. Нет лимита ходов/энергии-за-действие → можно гриндить ошибка→закрытие→ошибка бесконечно. Снэппет в `next.md` сам задал `_earned -= 1` — это дефект дизайн-дока. Фикс: `ERROR_PENALTY ≥ 5`, либо не платить полные 5$ за каскадные баги, либо лимит заработка за сессию.

**Замечание (не блокирует):** `WorkResult.errors_label` хардкодит «-%d$» = 1$/ошибка — верно для Bugfix (`ERROR_PENALTY=1`), неверно для Mail (−2/−5$). Косметика, с Slice I.

**Что добавлено:**

| Файл | Что |
|---|---|
| `scripts/run2/BugfixGame.gd` *(новый)* | Мини-игра «Фикс багов». `const BUGS` — пул из 12 багов (каждый: `bug`, `fix`, `traps`). 3 случайных бага на старте. Клик баг (подсветка янтарным `modulate`) → клик фикс. Правильный: +5$, баг закрыт. Ошибка: −1$, `_errors += 1`, `_spawn_new_bug()` (случайный баг не из активных). Все закрыты → `_finish_game()` (бонус +5$ если 0 ошибок). Багов > 5 → `_fail_game()` (`_earned = 0`). Результат через WorkResult + `work_day_finished`. `_energy_spent = 2` (списано в WorkHub). Колонки перестраиваются целиком (`_rebuild_columns`) на каждое действие. |
| `scenes/run2/BugfixGame.tscn` *(новый)* | Control full_rect, Bg бордовый. Margin → Layout(VBox): TopBar (BackBtn / Spacer / ProgressLabel «Багов: N» / EarnedLabel), GameArea (HBox: BugsColumn с красным заголовком «Баги» + BugsBox, ColumnGap, FixesColumn с зелёным «Фиксы» + FixesBox), FeedbackLabel. BugsBox/FixesBox наполняются кнопками программно. |
| `scenes/run2/WorkHub.tscn` *(изменён)* | Card_Bugfix переведена из locked в active: StyleBoxFlat_sorting (общий с Card_Sorting/Card_Mail), margins 12/8/12/8, NameLabel «Фикс багов», DescLabel «Сопоставь баг с правильным фиксом. Ошибка плодит новые баги.», StatsBox (8–25$ / -2 энергии / Риск: Высокий — красноватый), StartBtn «Начать». Неиспользуемый SubResource `StyleBoxFlat_locked` удалён (Godot сам выбросил при пересохранении). |
| `scripts/run2/WorkHub.gd` *(изменён)* | Вынесен общий `_start_work_game(scene_path, energy_cost)` — дублирование `_on_start_sorting` / `_on_start_mail_sorting` убрано, оба + новый `_on_start_bugfix` теперь однострочные обёртки. `BUGFIX_SCENE`, `BUGFIX_ENERGY_COST = 2`, `bugfix_start_button_path` @export. `_lock_mail_card()` обобщён в `_lock_card(card_name, start_btn)` — работает для любой карточки (Mail и Bugfix в Run 1). |
| `scripts/run2/WorkResult.gd` *(изменён)* | `setup()` получил 4-й опциональный параметр `comment_override: String = ""`. Если непустой — показывается вместо случайного из пула. Обратно совместимо: MailSortGame/WorkProgram вызывают с 3 аргументами. BugfixGame передаёт кастомный текст при провале. |

**Playtest через MCP (2 прогона, всё зелёное):**

*Прогон 1 — чистая победа (Run 2):* Desktop → WorkHub → Card_Bugfix активна (8–25$ / -2 энергии / Риск: Высокий) → «Начать» → энергия 8→6 → BugfixGame: 3 бага слева, 3 фикса справа, сатира внизу. Сопоставлены все 3 правильно (+5$ каждый, баг+фикс убираются, «Багов: 3→2→1»). WorkResult: «Заработано: +20$» (15 + бонус «Баг-слеер» +5), «Качество: Отлично», 0 ошибок. «Вернуться к работе» → WorkHub. ✓

*Прогон 2 — провал (Run 2):* «Начать» → энергия 6→4 → 3 неверных сопоставления подряд: «Багов: 3→4→5», −1$ каждое, каскад (новый баг появляется), фидбэк «Не тот фикс. Баг расплодился.». Шестой баг → провал: WorkResult «Заработано: +0$», «Ошибок: 3 (-3$)», «Качество: Провал», комментарий про аутсорс. «На рабочий стол» → Desktop. ✓

*Run 1:* WorkHub — и «Корпоративная почта», и «Фикс багов» заблокированы («Откроется позже», 🔒, StatsBox скрыт). ✓

`get_editor_errors` — чисто. Все 3 скрипта компилируются.

**Что keeper'у проверить вручную:**
- Тон 12 багов и фиксов — попадание в сатиру? Особенно «Агент продаёт переписку налево» / «Отозвать API-ключ агента», «Деньги уходят в минус» / «Добавить clamp в Economy.add», ловушки вроде «Сказать что это фича», «Назвать вечным двигателем».
- Баланс: за идеальный прогон 3×5 + 5 = 20$ при -2 энергии. За прогон с 1 ошибкой: 4×5 − 1 = 19$. Провал = 0$. Соотносится ли с таблицей раздела 7 work-expansion.md (bugfix $8–25)? Идеал даёт ровно 20$, но из-за каскада «худший успешный» прогон (закрыть 5 багов с 2 ошибками) даёт 5×5 − 2 = 23$ — то есть ошибки почти не штрафуют, потому что каждый лишний баг = +5$ потенциала. Возможно стоит увеличить `ERROR_PENALTY` или давать меньше за «лишние» баги.
- Сатирический комментарий провала — «Багов столько, что проект передали в аутсорс. Деньги — тоже.» (next.md просил «Передано в аутсорс» — смысл сохранён, текст развёрнут).
- Подсветка выбранного бага янтарным `modulate` Color(1, 0.84, 0.4) — достаточно заметна на тёмном фоне.

**Компромиссы / known issues:**
- **Economy не зачисляется.** BugfixGame (как и MailSortGame из Slice I) НЕ вызывает `Economy.add()` — `_earned` идёт только в WorkResult и `work_day_finished.emit()`. В playtest шапка показывала `$0` и после +20$. WorkProgram (сортировка) зачисляет напрямую через `_economy.add()`. Расхождение pre-existing с Slice I; next.md для Slice J явно описывает логику без Economy (snippet `_on_fix_picked` делает только `_earned += 5`). **Не чинил в этом слайсе** — это кросс-системное решение: либо чинить и Mail, и Bugfix, либо добавить централизованный обработчик `work_day_finished` → `Economy.add()` в Run-контроллерах (сейчас `Run1.gd._on_work_day_finished` / `Run2.gd._on_work_day_finished` = `pass`). Keeper'у решить.
- **`traps` в `BUGS` не используются.** Поле `traps` (3 ловушки на баг) присутствует в данных по образцу next.md, но в геймплее Slice J не задействовано — игра это чистое сопоставление 3↔3 (все 3 показанных фикса правильные, ошибка = выбрать фикс другого бага). Ловушки зарезервированы для апгрейда `stackoverflow_premium` (Slice L), который «исключает неправильный фикс из пула» — для этого в колонке фиксов должны быть и неправильные.
- **ShopApp.tscn был случайно повреждён и восстановлен.** `create_scene` не делает созданную сцену активной — первый `batch_add_nodes` ушёл в открытую ShopApp.tscn (17 нод + чужой скрипт + переименование). Восстановлено: `git checkout -- scenes/run2/ShopApp.tscn` + `EditorInterface.reload_scene_from_path()`. Проверено `get_scene_tree` — ShopApp в редакторе и на диске чистый (root script = `ShopApp.gd`, структура Bg→Margin→Layout). Keeper'у стоит на всякий случай открыть Магазин в Run 2.
- `_start_work_game(WORK_PROGRAM_SCENE, 0)` для сортировки → `spend_energy(0)` — безвредный no-op (RunService: energy не меняется, лишь эмитится лишний `energy_changed`). Сортировка тратит энергию по категориям внутри WorkProgram, поэтому стоимость старта = 0.
- Полная перерисовка колонок (`_rebuild_columns`) на каждое действие вместо точечного `remove_at`. Для ≤6 кнопок аллокация копеечная, зато нет рассинхрона индексов в забинденных лямбдах.
- BugfixGame не зачисляет ошибки в `RunService.register_work_error()` (как и MailSortGame). `_errors` локальный, идёт только в WorkResult. Если keeper хочет учёта ошибок в статистике дня — это отдельное решение, общее для всех мини-игр.

**Следующий слайс (Slice K — Meeting Simulator):** см. план work-expansion.md раздел 9.
...
**Вердикт:** Слайс принят. Код чистый, следует паттернам WorkProgram/WorkHub, контент писем — точное попадание в сатиру (сухая ирония, без переобъяснения). Дублирование `_on_start_sorting` / `_on_start_mail_sorting` — осознанное, на Slice J вынесем в общий метод.

**Замечания (не блокируют, на будущее):**
- Run 1: Card_Mail визуально зелёная (StyleBoxFlat_sorting), хотя кнопка disabled с 🔒. Стоит перекрашивать в серый через `add_theme_stylebox_override` в `_lock_mail_card()` — или просто пересоздать StyleBoxFlat_locked кодом. Сейчас терпимо.
- `_lock_mail_card()` использует жёсткие строковые пути вместо @export NodePath. Локальный хак для Run 1, не критично.

**Что добавлено:**

| Файл | Что |
|---|---|
| `scripts/run2/MailSortGame.gd` *(новый)* | Мини-игра «Корпоративная почта». Пул из 20 писем 5 типов: 10 рабочих, 3 ловушки начальника, 2 от AI-агента, 2 приглашения на митинг, 3 спама. 5–7 случайных писем за сессию. 4 папки: Срочно (+4$), Игнорировать (ловушка → −5$), Делегировать, Шаблон (+1$ нейтрально). Фидбек с задержкой 0.7с. Кнопка «Закончить» для досрочного завершения. Бонус +5$ за все правильные. Интеграция с WorkResult через `_show_work_result()` → `work_day_finished`. |
| `scenes/run2/MailSortGame.tscn` *(новый)* | Control full_rect, Bg тёмно-синий, TopBar (BackBtn / ProgressLabel / EarnedLabel / FinishBtn), EmailPanel с StyleBoxFlat (SenderLabel / SubjectLabel / BodyLabel), FeedbackLabel, FoldersBox с 4 кнопками равной ширины. |
| `scenes/run2/WorkHub.tscn` *(изменён)* | Card_Mail переведена из locked в active: StyleBoxFlat_sorting, margins 12/8/12/8, NameLabel (WHITE, font_size=20), DescLabel («Рассортируй письма по папкам. Игнорировать — тоже ответ.»), StatsBox (5–25$ / -2 энергии / Риск: Средний), StartBtn «Начать». Card_Bugfix осталась locked. |
| `scripts/run2/WorkHub.gd` *(изменён)* | `MAIL_SORT_SCENE`, `MAIL_ENERGY_COST = 2`, `mail_start_button_path` @export. `_on_start_mail_sorting()` — списывает 2 энергии, инстанциирует MailSortGame в GameHost, переподключает `_back_button`, подписывается на `work_day_finished`. `_update_energy()` — mail button disabled при energy < 2. Run 1: `_lock_mail_card()` скрывает StatsBox, показывает «Откроется позже», StartBtn → «🔒» disabled. |

**Smoke через MCP (5/5 зелёных):**
1. Desktop → WorkHub (Run 2) → Card_Mail активна (зелёная карточка, «Начать») ✓
2. «Начать» на Почте → MailSortGame загружена, письмо с отправителем/темой/текстом, 4 кнопки-папки ✓
3. «🚨 Срочно» на письме о баге → фидбек «Правильно», +4$ ✓
4. «📋 Шаблон» на письме о созвоне → фидбек «Сойдёт», +1$ ✓
5. «Закончить» → WorkResult (+5$, 0 ошибок, «Без ошибок. Подозрительно.») → «Вернуться к работе» → WorkHub ✓

**Что keeper'у проверить вручную:**
- Баланс наград: +4$ правильно / +1$ нейтрально / −2$ ошибка / −5$ ловушка / +5$ бонус за все правильные — соотносятся ли с экономикой (за полную сессию 5–7 писем максимум 4×7+5=33$, минимум −2×7=−14$).
- Энергия −2 за сессию (без per-email списания) — нормально ли, что Почта не списывает энергию за каждое письмо в отличие от Сортировки (−1 за категорию).
- Тон 20 писем — попадание в сатиру? Особенно ловушки начальника («Не игнорируй», «Будешь игнорировать — будешь искать работу») и AI-агенты («Автоответ: я в отпуске. Статус отпуска: бессрочный.»).
- Эмодзи в кнопках папок (🚨🗑👥📋) — ок в игровом интерфейсе или заменить на текст/спрайты?
- Run 1: карточка Почты показывает «🔒» disabled — достаточно ли понятно что откроется позже?
- Цвет RiskLabel «Риск: Средний» — желтоватый (Color(0.667, 0.667, 0.4)). Достаточно ли контрастен на тёмном фоне?

**Компромиссы / known issues:**
- Дублирование логики `_on_start_sorting` / `_on_start_mail_sorting` в WorkHub.gd. Код почти идентичен (разные сцены и стоимость энергии). На третьей игре (Фиксы, Slice J) — вынесем общий `_start_work_game(scene_path, energy_cost)`.
- NodePath в MailSortGame.tscn были дважды обёрнуты (`NodePath("NodePath(...)")`) из-за бага MCP `update_property` с парсингом строк. Исправлено прямым редактированием .tscn. При пересохранении сцены из редактора проблема не воспроизведётся (Godot сам пишет корректные NodePath).
- `MailSortGame._process` крутится каждый кадр пока `_awaiting_feedback == true`. Для одного таймера на 0.7с это копейки. При усложнении (много таймеров) — заменить на Timer-ноду.
- DatingProgram.tscn случайно получил мусорные MailSortGame-ноды при первом `batch_add_nodes` (баг MCP — добавляет в открытую сцену вместо созданной). Восстановлен через `git checkout`.

**Следующий слайс (Slice J — Bug Fixing Mini-game):** кандидат.
